...
import { ANDAR01_LAYOUT, ANDAR01_WALKABLES } from './andar01Layout';
...
export const Andar01: React.FC = () => {
    ...
    useEffect(() => {
        if (roomsData?.floorRooms) {
            const combinedRooms = ANDAR01_LAYOUT.roomLayouts.map((layout) => {
            ...
            console.log('✅ Salas carregadas:', combinedRooms.length);
        }
    }, [roomsData]);
    ...
    useEffect(() => {
        ...
        const gameLoop = () => {
            ...
            const viewW = ANDAR01_LAYOUT.canvasWidth / ZOOM_LEVEL;
            const viewH = ANDAR01_LAYOUT.canvasHeight / ZOOM_LEVEL;
            ...
            cam.x = Math.max(0, Math.min(cam.x, ANDAR01_LAYOUT.canvasWidth - viewW));
            cam.y = Math.max(0, Math.min(cam.y, ANDAR01_LAYOUT.canvasHeight - viewH));
            ...
            if (canvas) {
                ...
                if (ctx) {
                    ...
                    ctx.fillRect(0, 0, ANDAR01_LAYOUT.canvasWidth, ANDAR01_LAYOUT.canvasHeight);

                    ctx.scale(ZOOM_LEVEL, ZOOM_LEVEL);
                    ctx.translate(-cam.x, -cam.y);

                    // Draw corridors
                    ANDAR01_LAYOUT.corridors.forEach((c) => {
                    ...
                    // Draw doors
                    ANDAR01_LAYOUT.doors.forEach((d) => {
                        ctx.fillStyle = '#0f172a';
    ...
    if (roomsLoading) {
        return <div className="andar01-container">Carregando andar...</div>;
    }

    const currentRoomData = currentRoom ? JSON.parse(currentRoom.mapLayout || '{}') : null;

    return (
        <div className="andar01-container">
            <div className="andar01-sidebar">
                <header>
                    <h1 className="andar01-title">ANDAR 1</h1>
                    <p className="andar01-subtitle">Fundamental I - 1º Ano</p>
                </header>

                <div className="andar01-card">
                    <h2 className="andar01-card-title">Mapeamento Local</h2>
                    <div className="andar01-status">
                        <div className={`andar01-dot ${currentRoom ? 'active' : 'inactive'}`} />
                        <span className={`andar01-room-name ${currentRoom ? 'active' : 'inactive'}`}>
                ...
                {currentRoom && currentRoomData && (
                    <div className="andar01-card">
                        <img
                            ...
                            className="andar01-room-image"
                        />
                        <p className="andar01-room-info">{currentRoomData.info}</p>
                    </div>
                )}

                {currentRoom && currentRoomData && currentRoomData.actions && (
                    <div className="andar01-card">
                        <h2 className="andar01-card-title">Ações Inteligentes</h2>
                        <div className="andar01-actions-grid">
                            {/* Primeira ação: Acessar Sala */}
                            <button
                                className="andar01-action-btn"
                                onClick={() => {
                                    navigate(`/room/${currentRoom.id}`);
                                }}
                            >
                                <span className="andar01-action-icon">🚪</span>
                                <div className="andar01-action-content">
                                    <span className="andar01-action-label">Acessar Sala</span>
                                    <span className="andar01-action-command">Entrar no ambiente</span>
                                </div>
                            </button>
                            ...
                            {currentRoomData.actions.map((action: any, index: number) => (
                                ...
                                >
                                    <span className="andar01-action-icon">{action.icon}</span>
                                    <div className="andar01-action-content">
                                        <span className="andar01-action-label">{action.label}</span>
                                        <span className="andar01-action-command">{action.command}</span>
                                    </div>
                                </button>
                            ))}
                        </div>
                ...
                <div className="andar01-card">
                    <h2 className="andar01-card-title">Relatório de IA</h2>
                    <p className="andar01-text">"{aiState.response}"</p>
                </div>

                <div style={{ marginTop: 'auto' }}>
                    <button onClick={() => navigate('/player/character')} className="andar01-btn">
                        ← Voltar
                    </button>
                </div>
            </div>

            <div className="andar01-canvas-container">
                <canvas
                    ref={canvasRef}
                    width={ANDAR01_LAYOUT.canvasWidth}
                    height={ANDAR01_LAYOUT.canvasHeight}
                    className="andar01-canvas"
                />
            </div>
        </div>
    );
